...
def get_id_idx_dict(cnx):
    id_idx_dict = {}
    sql_str = "SELECT stations_lat_lon.INDEX, stations_lat_lon.ID FROM stations_lat_lon;"
    cursor = cnx.cursor()
...
def get_traffic_flow_piece(cnx, id_idx_dict, timestamp):
    n_feat = 1 + 3 # the first feat to flag vaild data
...
if __name__ == "__main__":
    cnx = connect_mysql(ip="192.168.188.222", port="3306", username="root", password="wonder")
    
    input_format = "%Y-%m-%d %H:%M:%S"
    output_format = "%Y-%m-%d_%H-%M-%S"
    
    start_datatime = datetime.strptime("2019-01-24 00:00:00", input_format)
    end_datetime = datetime.strptime("2019-01-31 23:55:00", input_format)
    cur_datetime = start_datatime
    while cur_datetime + timedelta(hours=23) + timedelta(minutes=55) <= end_datetime:
        ...
        traffic_flow_data, _, _ = get_traffic_flow_data(cnx, s_datetime, e_datetime, "5T")
        filename = f"{s_datetime.strftime(output_format)}-{e_datetime.strftime(output_format)}.npy"
        print("saved", filename)
        np.save(filename, traffic_flow_data)
